...
GameStateSandbox::GameStateSandbox(SDL_Window* window) : m_Window(window)
{
	...
	// Sandbox for initial entities
	{
		...
		testEntity->AddComponent(Component::AnimatedSprite(
		                             Assets::Factory::GetAnimation(Assets::ANIM_WIZARD_IDLE)));
...
std::unique_ptr<GameState> GameStateSandbox::Update(ECS::DeltaSeconds d)
{
	...
	m_TextTarget->QueueText("ABCDEFGHIJKLM", glm::vec2{ -2.f, -3.5f },
	                        Color::DarkGreen, 0.5f);
	m_TextTarget->QueueText("NOPQRSTUVWXYZ", glm::vec2{ -2.f, -2.f },
	                        Color::DarkGreen, 0.5f);
